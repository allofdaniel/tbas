...
export default function useNotamLayer(
  ...
  useEffect(() => {
    ...
    const setCursor = () => { map.current.getCanvas().style.cursor = 'pointer'; };
    const resetCursor = () => { map.current.getCanvas().style.cursor = ''; };
  ...
  }, [mapLoaded, notamLocationsOnMap, notamData, is3DView]);
}
